...
func decryptMsg(msg []byte, pubKey key.MachinePublic, privKey key.MachinePrivate) ([]byte, error) {
...
func encode(v interface{}, pubKey key.MachinePublic, privKey key.MachinePrivate) ([]byte, error) {
	...
	log.Debug().
		Str("func", "encodeMsg").
		Bytes("payload", b).
		Msg("Response to be sent to tailscale client.")
...
func encodeMsg(
	...
	return privKey.SealTo(pubKey, payload), nil
}
...
func ParseNodeKey(textKey string) (key key.NodePublic, err error) {
	const prefix = `nodekey:`
	if strings.HasPrefix(textKey, prefix) {
		err = key.UnmarshalText([]byte(textKey))		
	} else {
		err = key.UnmarshalText([]byte(prefix+textKey))		
	}
	return
}

func ParseMachineKey(textKey string) (key key.MachinePublic, err error) {
	const prefix = `mkey:`
	if strings.HasPrefix(textKey, prefix) {
		err = key.UnmarshalText([]byte(textKey))		
	} else {
...
func ParseDiscoKey(textKey string) (key key.DiscoPublic, err error) {
	const prefix = `discokey:`
	if strings.HasPrefix(textKey, prefix) {
		err = key.UnmarshalText([]byte(textKey))		
	} else {
		err = key.UnmarshalText([]byte(prefix+textKey))		
